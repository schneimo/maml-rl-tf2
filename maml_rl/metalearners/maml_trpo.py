...
class MetaLearner(BaseMetaLearner):
    ...
    def kl_divergence(self, episodes, old_pis=None):
        ...
        for (train_episodes, valid_episodes), old_pi in zip(episodes, old_pis):
            ...
            kl = weighted_mean(distributions.kl_divergence(pi, old_pi), axis=0, weights=mask)
            kls.append(kl)
    ...
    def hessian_vector_product(self, episodes, damping=1e-2):
        ...
        def _product(vector):
            ...
            with tf.GradientTape as outter_tape:
                ...
                # First derivative
                grads = inner_tape.gradient(kl, self.policy.trainable_variables)
                flat_grad_kl = flatgrad(grads)
                grad_kl_v = tf.tensordot(flat_grad_kl, vector, axes=1)
            # Second derivative
            grad2s = outter_tape.gradient(grad_kl_v, self.policy.trainable_variables)
            flat_grad2_kl = flatgrad(grad2s, self.policy.trainable_variables)
    ...
    def step(self,
        ...
        shs = tf.tensordot(stepdir, hessian_vector_product(stepdir), axes=1)
        lagrange_multiplier = tf.math.sqrt(2 * kl_limit / (shs + 1e-10))
